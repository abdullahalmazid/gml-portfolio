...
export default function CompactMode({ section, items = [] }) {
  ...
  return (
    <ol
      className={`${twoColumns ? 'grid md:grid-cols-2 gap-x-12' : ''
        ...
        return (
          <li key={item.id}>
            <MotionDiv delay={Math.min(i, 8) * 0.04}>
              <div className="group flex gap-4 items-baseline py-4 -mx-3 px-3 rounded-lg hover:bg-black/[0.02] dark:hover:bg-white/[0.03] transition-colors">
                <CardLink href={parts.href} className="contents">
                  {showIndex && (
                ...
                {/* Outside the CardLink: nested anchors break hydration. */}
                {external && (
                  <a
                    href={external.href}
                    target="_blank"
